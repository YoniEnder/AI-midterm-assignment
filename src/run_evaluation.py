...
from src.evaluation_suite import get_test_cases
...
def run_evaluation():
    ...
    print("Initializing Evaluator (Judge: GPT-4)...")
    evaluator = SystemEvaluator(judge_model="gpt-4")
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if system_result.get('timeline_tool_used'):
                print("Timeline Tool Used: Yes")
            print()
            ...
            # Display results
            print("\n📊 Evaluation Results:")
            print(f"  Correctness: {evaluation['correctness']['score']:.2f} - {evaluation['correctness'].get('reasoning', 'N/A')[:100]}")
            print(f"  Relevancy: {evaluation['relevancy']['score']:.2f} - {evaluation['relevancy'].get('reasoning', 'N/A')[:100]}")
            print(f"  Recall: {evaluation['recall']['score']:.2f} - {evaluation['recall'].get('reasoning', 'N/A')[:100]}")
            ...
            results.append({
                ...
                "system_result": {
                    ...
                    "timeline_tool_used": system_result.get("timeline_tool_used", False),
                },
    ...
    if successful_results:
        ...
        relevancy_scores = [
            r["evaluation"]["relevancy"]["score"]
            for r in successful_results
        ]
        recall_scores = [
            r["evaluation"]["recall"]["score"]
            for r in successful_results
        ]
        overall_scores = [
        ...
        print(f"Routing Accuracy: {routing_correct}/{len(successful_results)} ({routing_correct/len(successful_results)*100:.1f}%)")
        ...
        print(f"Index Selection Accuracy: {index_correct}/{len(successful_results)} ({index_correct/len(successful_results)*100:.1f}%)")
        print()
